...
with st.expander("🧭 知識點一：自動數數機——range() 函式的三大密碼", expanded=True):
    ...
    rq1, rq2, rq3, rq4, rq5 = st.tabs(["📌 數數題 一", "📌 數數題 二", "📌 數數題 三" , "📌 實作題 一" , "📌 實作題 二"])
    with rq1:
    ...
    with rq4:
        ...
        with st.expander("👀 查看題目"):
            st.caption("輸入第一行是一個數字n，代表接下來有n行，每行一個數字，請輸出這n個數字總共有幾個偶數，最多連續出現幾個偶數")
            st.write("ex input:")
            st.write("4")
            st.write("2")
            st.write("1")
            ...
            st.write("2")
    with rq5:
        st.markdown("[b762. 英國聯蒙](https://zerojudge.tw/ShowProblem?problemid=b762)")
...
for i in range(0, len(scores), 3):
    print(scores[i])
        ...
        with st.expander("👀 查看解答"):
            st.success("**答案：\\n90\\n60**")
            st.caption("分析：len(scores) 是 4，range(0, 4, 3) 會產生號碼牌 0 和 3。scores[0] 是 90 skepticism，scores[3] 是 60。")
