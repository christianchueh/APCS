...
with st.expander("🧭 知識點一：自動數數機——range() 函式的三大密碼", expanded=True):
    ...
    rq1, rq2, rq3, rq4 = st.tabs(["📌 數數題 一", "📌 數數題 二", "📌 數數題 三" , "📌 實作題 一" ])
    with rq1:
    ...
    with rq4:
        st.markdown("**題目：連續幾個、總共幾個？**")
        st.caption("輸入第一行是一個數字 n，代表接下來有 n 行，每行一個數字。請輸出這 n 個數字總共有幾個偶數，以及最多連續出現幾個偶數。")
        
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**範例輸入 (Input)**")
            st.code("4\n2\n1\n4\n8", language="text")
        with col2:
            st.markdown("**範例輸出 (Output)**")
            st.code("3\n2", language="text")
            
        with st.expander("👀 查看參考解答範例 (Python)"):
            st.markdown("這道題需要用 `range(n)` 來控制讀取資料的次數，並用計數器紀錄狀態：")
            st.code("""
# 讀取總共有幾個數字
n = int(input())

total_evens = 0      # 總偶數個數
max_continuous = 0   # 最大連續偶數個數
current_continuous = 0 # 目前連續偶數個數
...
# ==========================================
# 🆕 補強過渡章節：for 迴圈與 range 的完美結合
# ==========================================
with st.expander("💡 觀念補強：當 `for` 遇上 `range()`", expanded=True):
    st.markdown("""
    剛剛我們看完了 `range()` 自動吐出數字的規則，但它要怎麼跟迴圈搭配呢？
    在 Python 中，我們最常用 `for i in range(...)` 來讓程式**重複執行指定的次數**，同時變數 `i` 就會依序變成 `range` 吐出來的每一個數字。
    
    ### 🌟 經典教學範例：* 星號直角三角形
    當我們要印出由小變大的星星圖形時，利用 `range()` 的數字變化來控制每行星星的數量是最完美的做法！
    """)
    
    # 畫出概念圖或表格
    st.markdown("##### 📐 三角形層數與星星數量的關係 (以 4 層為例)")
    st.table([
        {"第幾輪迴圈 (i)": "i = 1", "程式做的事情": "print('*' * 1)", "畫面輸出的成果": "*"},
        {"第幾輪迴圈 (i)": "i = 2", "程式做的事情": "print('*' * 2)", "畫面輸出的成果": "**"},
        {"第幾輪迴圈 (i)": "i = 3", "程式做的事情": "print('*' * 3)", "畫面輸出的成果": "***"},
        {"第幾輪迴圈 (i)": "i = 4", "程式做的事情": "print('*' * 4)", "畫面輸出的成果": "****"}
    ])
    
    st.markdown("**🎬 實作程式碼：**")
    st.code("""
n = 4
# 因為「顧前不顧後」，如果想數到 n，結束值必須寫 n + 1 喔！
...
print(ans)
        ...
        with st.expander("👀 查看解答"):
            ...
            * 產生的號碼牌 `i` 分別為：`4, 3, 2`。
            * 對應的資料：`data[4]`是 5，`data[3]`是 4，`data[2]`是 3。
            * 累加結果：5 + 4 + 3 = 12。
            """)
...
with st.expander("🧭 知識點四：控制迴圈的煞車與油門——break 與 continue"):
    ...
    * **`continue`（跳過這回合）**：**只結束這一次**執行，直接回到迴圈開頭準備數下一個數字。
    ...
    st.table([
        {"關鍵字": "break", "功能說明": "毀滅整個迴圈", "實際行為": "一看到 it，後面所有次數都不數了，迴圈原地解散。"},
