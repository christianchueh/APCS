...
with st.expander("🧭 知識點三(上)：複合條件的黏著劑——邏輯運算子 (Logical Operators)"):
    ...
    # ---------------------------------------------------------
    # 新增：APCS 比較 x 邏輯運算子 8 題魔鬼特訓區（使用 Tabs 呈現）
    # ---------------------------------------------------------
    st.markdown("---")
    st.markdown("### 🧠 APCS 觀念奠基：比較 $\\times$ 邏輯複合閱讀特訓（8題全開）")
...
print(ans)
        """, language="python")
        q3_1 = st.radio("請問最終印出的 `ans` 是什麼？", ["True", "False", "10", "5"], key="q3_1")
        with st.expander("🔍 查看【題一解析】"):
            st.write("**正確答案：`False`**")
            st.markdown("> **解析**：\n> 1. `a > 5` $\rightarrow$ `10 > 5` 成立，得到 **`True`**。\n> 2. `b == 3` $\rightarrow$ `5 == 3` 不成立，得到 **`False`**。\n> 3. `True and False` 結果為 **`False`**（`and` 只要有一邊錯就是錯）。")
...
print(pass_lesson)
        """, language="python")
        q3_2 = st.radio("請問最終印出的結果是什麼？", ["True", "False", "45", "報錯（Error）"], key="q3_2")
        with st.expander("🔍 查看【題二解析】"):
            st.write("**正確答案：`True`**")
            st.markdown("> **解析**：\n> 1. `score >= 60` $\rightarrow$ `45 >= 60` 得到 **`False`**。\n> 2. `is_teacher_pet` 為 **`True`**。\n> 3. `False or True` 最終得到 **`True`**（`or` 只要有一邊對就全對）。")
...
print(is_minor)
        """, language="python")
        q3_3 = st.radio("請問最終印出的結果是什麼？", ["True", "False", "15", "18"], key="q3_3")
        with st.expander("🔍 查看【題三解析】"):
            st.write("**正確答案：`True`**")
            st.markdown("> **解析**：\n> 1. 內層 `age >= 18` $\rightarrow$ `15 >= 18` 得到 **`False`**。\n> 2. 外層 `not False` 反轉後得到 **`True`**。")
...
print(result)
        """, language="python")
        q3_4 = st.radio("請問最終印出的結果是什麼？", ["True", "False", "0", "12"], key="q3_4")
        with st.expander("🔍 查看【題四解析】"):
            st.write("**正確答案：`True`**")
            st.markdown("> **解析**：\n> 1. `num % 3` 得到 `0`。比較算式 `0 != 0`（0不等於0）不成立，得到 **`False`**。\n> 2. 後半段 `num > 10` $\rightarrow$ `12 > 10` 成立，得到 **`True`**。\n> 3. `False or True` 得到 **`True`**。")
...
print(result)
        """, language="python")
        q3_5 = st.radio("請問這段程式在 Python 中會如何？", ["True", "False", "報錯（不允許連續比較）", "7"], key="q3_5")
...
print(result)
        """, language="python")
        q3_6 = st.radio("請問最終結果為何？", ["True", "False", "None", "報錯"], key="q3_6")
        with st.expander("🔍 查看【題六解析】"):
            st.write("**正確答案：`True`**")
            st.markdown("> **解析**：**`and` 的優先級比 `or` 高**！\n> 1. 電腦會先處理後半段的 `False and False` $\rightarrow$ 得到 `False`。\n> 2. 算式簡化成 `True or False` $\rightarrow$ 最終得到 **`True`**。\n> 如果這題從左到右算就會錯，APCS 超愛挖這個坑！")
...
print(result)
        """, language="python")
        q3_7 = st.radio("請問這段程式會輸出什麼？", ["True", "False", "程式大崩潰（TypeError）", "10"], key="q3_7")
...
print(ans)
        """, language="python")
        q3_8 = st.radio("試著當看看電腦，最後算出的結果是？", ["True", "False", "報錯", "23"], key="q3_8")
        with st.expander("🔍 查看【題八解析】"):
            st.write("**正確答案：`True`**")
            st.markdown("""
            > **我們分三路大軍擊破它：**
            > 1. **第一部分**：`not (x * 2 == y + 1)` $\rightarrow$ `not (16 == 16)` $\rightarrow$ `not True` 得到 **`False`**。
            > 2. **第二部分**：`(y % x == 7)` $\rightarrow$ `15 % 8` 餘數是 `7`，`7 == 7` 成立，得到 **`True`**。
            > 3. **第三部分**：`(x > 5)` $\rightarrow$ `8 > 5` 成立，得到 **`True`**。
            > 
