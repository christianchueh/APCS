...
* 關鍵在於：**在 Python 中，如果一個函式執行完畢卻沒有遇到任何 `return` 指令，它會默認回傳 `None`**。因此 `res2` 接收到的值是 `None`！""")
        ...
        st.error("❌ 答錯囉！請特別注意 Python 函式沒有寫 return時的預設回傳機制。")
...
從這一刻起，內部 `data` 的任何修改（改值、append）都只發生在副本上，**切斷了與外部 my_list 的連結**。所以外面的 `my_list` 依然完好如初，長度仍為 2。""")
        ...
        st.error("❌ 哇！踩中陷阱了！請仔謝看第一行 data.copy() 對記憶體關係做了什麼重大的改變。")
...
st.info("""
* **【第一階段：瘋狂向下呼叫（疊高置物櫃）】**
    * 呼叫 `fact(3)` ➡️ 發現 `n != 1` ➡️ 暫停，等待 `fact(2)` 的答案。 (此時記憶體記住：3 * 未知)
    * 呼叫 `fact(2)` ➡️ 發現 `n != 1` ➡️ 暫停，等待 `fact(1)` 的答案。 (此時記憶體記住：2 * 未知)
...
2. `g(2, 2)`：因為 2 不大於 2 (不滿足 x > y)，執行 `g(2, 0) + 1`
