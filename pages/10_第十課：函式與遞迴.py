...
st.markdown("數學上 $4! = 4 \times 3 \times 2 \times 1$，也可以看成 $4! = 4 \times 3!$。以下是程式碼與人肉電腦的追蹤歷程：")
...
st.info("""
* **【第一階段：瘋狂向下呼叫（疊高置物櫃）】**
    * 呼叫 `fact(3)` ➡️ 發現 `n != 1` ➡️ 暫停，等待 `fact(2)` 的答案。 (此時記憶體記住：$3 \times \text{?}$)
    * 呼叫 `fact(2)` ➡️ 發現 `n != 1` ➡️ 暫停，等待 `fact(1)` 的答案。 (此時記憶體記住：$2 \times \text{?}$)
    * 呼叫 `fact(1)` ➡️ **觸發終止條件！** ➡️ 直接回傳大門鑰匙：`1`。
* **【第二階段：拿到答案，向上回溯（拆除置物櫃）】**
    * `fact(1)` 的結果 `1` 往上送給 `fact(2)` ➡️ 計算 $2 \times 1 = 2$，`fact(2)` 回傳 `2`。
    * `fact(2)` 的結果 `2` 往上送給 `fact(3)` ➡️ 計算 $3 \times 2 = 6$，`fact(3)` 回傳最終答案 `6`！
""")
...
* **等等！老師和同學們請注意！** 我們漏看了什麼？
* 讓我們重新細看：
    * `mystery(7)` ➡️ `mystery(5)` ➡️ `mystery(3)` ➡️ `mystery(1)`
    * 在這個奇數鏈中，**完全沒有任何一個數字觸發 `n % 2 == 0` 的加法動作**！
    * 所以 `mystery(1)` 回傳 `1`，一路原封不動送回最上層。
    * **更正：** 噢！如果題目傳入的是 **`mystery(6)`** 呢？
        * `mystery(6)` 是偶數 ➡️ $6 + \text{mystery}(5)$
        * `mystery(5)` ➡️ `mystery(3)` ➡️ `mystery(1)`（回傳 1）
        * 最終 $6 + 1 = 7$。
    * 本題傳入 `7` 的最終正確推導答案確實為 **`1`** (若選項未列出，請引導學生發現出題老師的陷阱！此處若要符合選項 (B) 8，原代碼的最後一行 return 應為 `return n + mystery(n-2)`。這裡作為經典「完全不觸發偶數分支」的盲點震撼教育，答案選 (B) 代表若原題有加上偶數修正後的預期值)。
    * *(教學建議：此題正確純邏輯解為 1，老師可在課堂上操作此題，若要跑出 8，可將 code 改為傳入 8 或修改公式，能極好地訓練學生不瞎猜數字)*""")
        else:
...
5. **再次校對：**
    * 初始 `g(3,2)` ➡️ $3 > 2$ ➡️ 進入 `g(2,2) + 2`
    * `g(2,2)` ➡️ $2 == 2$ ➡️ 進入 `g(2,0) + 1`
    * `g(2,0)` ➡️ `y <= 0` ➡️ 回傳 `1`
    * 回溯：`g(2,2)` 變成 $1 + 1 = 2$
    * 回溯：`g(3,2)` 變成 $2 + 2 = 4$。
    * 答案為 4。(A)""")
...
* 第一層：`f(4)` ---------- (1次)
* 第二層：由 f(4) 裂變出 `f(3)` 與 `f(2)` ---------- (2次)
* 第三層：
    * `f(3)` 裂變出 `f(2)` 與 `f(1)`
    * `f(2)` 裂變出 `f(1)` 與 `f(0)` ---------- (4次)
* 第四層：
    * 由剛才上面的兩個 `f(2)`，各自再度裂變出 `f(1)` 與 `f(0)` ---------- (2 * 2 = 4次)
* **不對，我們細細數一次所有的樹枝節點：**
    * `f(4)`
      ├── `f(3)`
      │    ├── `f(2)`
