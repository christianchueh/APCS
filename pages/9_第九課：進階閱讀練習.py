...
st.caption("適用 Python 3.14 環境 | 15 題符合進階閱讀練習題（已修正 Bug 與超出範圍之內容）")
st.markdown("👉 **請仔細閱讀程式碼並追蹤變數變化，點選選項後系統將即時回饋正誤與詳細解析！**")
st.markdown("---")

# ==============================================================================
# Q1 - Q15 題目與即時反饋系統（嚴格限制教學範圍內）
# ==============================================================================
...
* 組合成的字串即為 `'y_62'` 的倒數對應字元，即實質抓到為 `'h'`（第11格）、`'P'`（第9格）、`'_'`（第7格）、`'2'`（第5格），拼起來就是 `'hP_2'`。""")
    else:
...
if q10_ans:
    if q10_ans.startswith("(B)"):
        st.success("🎉 答對了！")
        st.info("💡 **解析**：字串 `"123a5"` 中包含英文字母 `'a'`，因此 `data.isdigit()` 的結果為 `False`。程式會執行 `else` 區塊，進行字串的接合（Concatenation）：`"123a5" + "5"` 得到 `"123a55"`。")
    else:
...
if q12_ans:
    if q12_ans.startswith("(A)"):
        ...
        st.info("💡 **解析**：\n* `s[:2]` 省略起點代表從頭開始，取到索引 1（不含2），即 `'Py'`。\n* `s[4:]` 省略終點代表一路取到最後，從索引 4 開始取（`'o'`, `'n'`），即 `'on'`。\n兩者相加拼接後，結果為 `'Pyon'`。")
    else:
...
if q13_ans:
    if q13_ans.startswith("(B)"):
        st.success("🎉 答對了！")
        st.info("💡 **解析**：`sum(scores)` 計算總和為 $10+20+30+40 = 100$。`len(scores)` 的長度為 $4$。進行 `100 // 4` 整除運算（無條件捨去至整數），得到的結果是整數 `25`。")
    else:
...
if q14_ans:
    if q14_ans.startswith("(D)"):
        ...
        st.error("❌ 答錯囉！請先判斷 20 有沒有在陣列裡面。")

st.markdown("---")

# --- Q15 ---
st.subheader("💡 Q15. 字串的大小寫轉換與元素比對（Bug 修正版）")
...
if q15_ans:
    if q15_ans.startswith("(A)"):
        ...
        st.info("💡 **解析**：`s1.lower()` 會把 `"Apcs"` 轉換為全小寫的 `"apcs"`；同樣地，`s2.lower()` 也會把 `"APCS"` 轉換為全小寫的 `"apcs"`。因為轉換後的兩字串內容完全相同，所以使用 `==` 比對的結果為 `True`。這常用於做不區分大小寫的檢查。")
    else:
        st.error("❌ 答錯囉！請把兩邊都試著轉換成全小寫之後再進行字串比對。")

st.markdown("---")
st.balloons()
st.markdown("### 🏁 綜合挑戰結束！請老師根據學生的答對回饋來進行盲點總結與講評喔！")
